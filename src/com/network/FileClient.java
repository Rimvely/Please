...
public class FileClient {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// d:\\doc\\a.text
			System.out.print("전송할 파일경로명?");
			path = scn.next();

			File f = new File(path); // File 클래스생성
			...
			info = new FileInfo();
			info.setCode(100);
			info.setData(f.getName().getBytes()); // 파일이름을 바이트로 바꿔주는것.
			info.setSize((int) f.length()); // length로 바꿔서

			oos.writeObject(info);
			System.out.println(f.getName() + " 파일전송시작...");

			Thread.sleep(30); // 계속 쉼없이 보내면 렉이걸려서 조금 쉬면서 보내게만듬
